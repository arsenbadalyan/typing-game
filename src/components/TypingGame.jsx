...
const TypingGame = () => {
    ...
    return (
        ...
        <div className="typing-game-container">
            <div className={`typing-game${paragraph === typed ? ' winner' : time === 0 ? ' time-out' : time <= 10 ? ' time-running-out' : focused ? ' focused' : ''}`}>
                <div className="game-data">
                    <div>
                        Mistakes: <strong>{paragraph.slice(0, typed.length).split('').filter((letter, index) => typed.charAt(index) !== letter ).length}</strong>
                    </div>
                ...
                >
                    {paragraph.slice(0, typed.length).split('').map((letter, index) => (
                        // Check if the letter is the same as the typed letter
                        <span
                            key={index}
                            className={typed.charAt(index) === letter ? 'correct' : 'incorrect'}
                        >
                            {letter}
                        </span>
                    ))}
